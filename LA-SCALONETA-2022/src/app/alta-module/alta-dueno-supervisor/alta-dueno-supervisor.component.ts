...
export class AltaDuenoSupervisorComponent implements OnInit {
  ...
  public async registarAutoridad()
  {
    ...
    {
      //Valido datos.
      let datoInvalido = this.srvFirebase.validarAutoridadDB(
      ...
      if (datoInvalido == "ninguno")
      {
        ...
        let resultadoRegistro = await this.srvAuth.registerSinLogearse(this.correo_autoridad, this.password_autoridad, this.sonidoActivado);
        //let resultadoRegistro = "ok";
  
        if (resultadoRegistro == "ok")
        {
          this.srvFirebase.subirAutoridadDB(
          this.correo_autoridad,
          this.password_autoridad,
          this.nombre_autoridad,
          this.apellidos_autoridad,
